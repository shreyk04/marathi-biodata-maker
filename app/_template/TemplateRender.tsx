...
function TemplateRender( { backgroundImageSrc, height,left_offset, withPhoto, isFinal=false, form }:  { backgroundImageSrc: string, height: string,left_offset:string,withPhoto?:boolean, isFinal?:boolean , form: any}) {
    ...
    return (
        ...
        <div className={`w-96 mt-3 h-[80vh] relative   mx-auto px-8  rounded-lg  `} ref={templateRef} style={{ backgroundImage: `url(${backgroundImageSrc})`, backgroundPosition: "center", backgroundRepeat: "no-repeat", backgroundSize: "contain",minHeight: height || "100vh",  // ✅ Ensures proper height
           height: "auto" }}>
            {
                withPhoto && <div className={`absolute top-[20%] right-[80px] border ${!uploadedPhoto? "border border-red-300" : ""}  bg-slate-100 backdrop-opacity-55 shadow-sm w-[10%] h-[10%]`}   style={{backgroundPosition:"center",backgroundRepeat:"no-repeat",backgroundSize:"cover"}}>
                    <img src={uploadedPhoto || "/images/avatar-profile-icon.jpg"} className='w-full h-full' alt="" />
            </div>
            }
...

                {
                   Object.keys(form).map((key) => {
                    ...
                    return (
                        ...
                        <div key={key} className='flex text-[7px]' style={{paddingLeft: left_offset}} >
